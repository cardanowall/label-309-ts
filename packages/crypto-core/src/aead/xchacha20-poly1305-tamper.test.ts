...
const NONCE = seedBytes('cardanowall-aead-tamper-nonce-2-5-xchacha', 24);
const AAD = new TextEncoder().encode('cardano-poe-kek-v1');
const PLAINTEXT = new TextEncoder().encode(
  'sealed-poe tamper-test plaintext - 2-5 - deterministic - xchacha variant',
);

const CIPHERTEXT_WITH_TAG = xchacha20Poly1305Encrypt({
  key: KEY,
  nonce: NONCE,
  aad: AAD,
  plaintext: PLAINTEXT,
...
describe('xchacha20Poly1305Decrypt — tamper resistance (AeadVerificationError on any mutation)', () => {
  it('round-trips the unmutated ciphertext successfully (sanity)', () => {
    const recovered = xchacha20Poly1305Decrypt({
      key: KEY,
      nonce: NONCE,
      aad: AAD,
