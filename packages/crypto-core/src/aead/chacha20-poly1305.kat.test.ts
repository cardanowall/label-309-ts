...
interface ChaCha20Poly1305KatVector {
  name: string;
  key_hex: string;
  nonce_hex: string;
  aad_hex: string;
...
interface ChaCha20Poly1305KatCorpus {
  version: number;
  primitive: string;
  source: string;
  vectors: ChaCha20Poly1305KatVector[];
}
...
const katFixturePath = path.resolve(
  here,
  '../../tests/fixtures/aead/chacha20-poly1305-rfc8439-kat.json',
);
const roundtripFixturePath = path.resolve(
  here,
  '../../tests/fixtures/aead/chacha20-poly1305-roundtrip.json',
);
const katCorpus = JSON.parse(fs.readFileSync(katFixturePath, 'utf8')) as ChaCha20Poly1305KatCorpus;
const roundtripCorpus = JSON.parse(
  fs.readFileSync(roundtripFixturePath, 'utf8'),
) as ChaCha20Poly1305KatCorpus;

describe('chacha20Poly1305 — KAT vectors (RFC 8439 §2.8.2)', () => {
  for (const vector of katCorpus.vectors) {
    it(`encrypts + decrypts byte-identically for ${vector.name}`, () => {
      ...
      const ct = chacha20Poly1305Encrypt({ key, nonce, aad, plaintext });
      expect(bytesToHex(ct)).toBe(vector.expected_ciphertext_with_tag_hex);

      const recovered = chacha20Poly1305Decrypt({ key, nonce, aad, ciphertext: expected });
      expect(bytesToHex(recovered)).toBe(vector.plaintext_hex);
    });
  }
});

describe('chacha20Poly1305 — roundtrip parity vectors (project-internal)', () => {
