...
import type { Cip309ClientConfig, FetchImpl } from './types';
 ...
 * infer the URL — any CIP-309 gateway may issue keys in its own format. Omit it
 * for anonymous read-only usage.
 */
function resolveBaseUrl(config: Cip309ClientConfig): string {
  const baseUrl = config.baseUrl?.trim();
  if (baseUrl === undefined || baseUrl === '') {
    throw new InvalidClientConfigError(
      'Cip309Client: baseUrl is required. Pass the base URL of the CIP-309 ' +
